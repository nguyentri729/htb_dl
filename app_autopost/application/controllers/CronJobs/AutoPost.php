...
class AutoPost extends CI_Controller {
	...
	public function index()
	{
		...
		$this->db->where("posts.num_repeat >= 0");
	...
	public function PostDone($id){
		...
		if($posts->num_rows() > 0){
			...
			if($result_post['time_repeat'] > 0){
				$time_repeat = $result_post['time_repeat'];
				if($result_post['num_repeat'] > 0){
					$this->db->query("UPDATE posts SET time_repeat = ".time()." + {$time_repeat}, num_repeat = num_repeat - 1 WHERE id  = $id");
				}else{
					$this->db->query("UPDATE posts SET posted = 1 WHERE id  = $id");
				}
