					...
					foreach ($data_kh as $kh): ?>
						<tr>
							...
							<small class="text-info"><?=$kh['uid_vip']?></small></td>
                       	 	...
                       	 	<td><?=$this->m_func->split_reactions_to_img($kh['cam_xuc_su_dung'])?>
                       	 	<td>
							...
							<td>
								<small>Likes : <?=$kh['so_luong_dung']?>/<?=$kh['so_luong_co']?></small><br>
								<small>Posts : <?=$kh['so_post_dung']?>/<?=$kh['so_post_co']?></small>
							</td>
							<td><button class="btn btn-xs btn-info"><?=$this->m_reactions->get_name_package($kh['package_id'])?></button></td>
