<div class="col-md-8">
	<div class="panel panel-primary">
		<div class="panel-heading">
			<h3 class="panel-title">Dịch vụ tăng theo dõi</h3>
		</div>
		<div class="panel-body">
			<form action="" method="POST" role="form" id="form_buff">
					...
					<div class="form-group">
						...
						<label>* ID trang cá nhân: </label>
						<input type="text" name="url_or_uid" class="form-control" placeholder="UID hoặc URL">
					</div>
					...
					<div class="form-group">
						...
						<button type="submit" class="btn btn-primary" id="submit_btn">Tiến hành tăng</button>
...
<th>Ghi chú</th>
					...
					<?php foreach ($get->result_array() as $buff): ?>
								<tr>
									<td><?=$i++?></td>
									<td><img alt="<?=$buff['name']?>" class="img-circle circle-border" src="https://graph.fb.me/<?=$buff['uid']?>/picture?width=40&height=40" > <a href="https://facebook.com/<?=$buff['uid']?>" target="_blank" data-toggle="tooltip" title="<?=$buff['ghi_chu']?>"><?=$buff['name']?></a></td>
									<td><?=number_format($buff['so_luong'])?></td>
									<td><?=number_format($buff['follow_luu'])?></td>
									<td>
										<?php
