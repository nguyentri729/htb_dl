...
class Login extends CI_Controller {
	public function __construct(){
	    ...
	    $this->load->model('quanli/m_login', 'm_login');

	}
	public function index()
	{
		if($this->m_func->check_login('quanli') == TRUE){
			//echo $this->session->userdata('quanli_id');
			redirect('/QuanLi/Dashboard');
			exit;
			...
			if($this->input->post()){
					...
					if ($this->form_validation->run() == TRUE) {
						...
						if($check_captcha['status']){
							...
							$password = $this->m_login->creat_pass($this->input->post('password'));
	
							$this->db->where('email', $email);
							$this->db->where('password', $password);
							$get = $this->db->get('quanli_acc');
							if($get->num_rows() > 0){
								...
								if($get->result_array()[0]['active'] == 0){
									...
									$this->m_func->noti(false, 'Tài khoản của bạn đã bị khóa, vui lòng liên hệ với QTV để được hỗ trợ.');
										...
										$login_session = array(
											'quanli_id' => $get->result_array()[0]['id']
										);
			...
			$this->load->view('QuanLi/login', $data, FALSE);	
		}
...
/* Location: ./application/controllers/QuanLi/Login.php */