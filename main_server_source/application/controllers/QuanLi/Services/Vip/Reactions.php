	...
	public function index()
	{
		...
		if($this->input->get('chinh_sua') != ''){
			...
			if($get->num_rows() > 0){
				...
				//get info ctv
				$this->info_member = $this->m_quanli->get_info_ctv($data_khachhang['user_creat']);
				...
				if($this->input->post('name_vip') !=''){
					...
					if ($this->form_validation->run() == TRUE) {
						...
						if($this->db->update('vip_reactions', $arrayName)){
							if($this->m_server->update($link_server.'/api/vip_reactions.php', $arrayName, $data_khachhang['id'])){
									$this->m_func->wait_server('add');
				...
				$data = array(
					...
					'view' => 'QuanLi/Service/Vip/edit_reactions'
				);
			...
			$data =  array(
				...
				'data'  => array(
					...
					'data_kh' => $this->db->get('vip_reactions')->result_array()
				),
				'assets' => array(
				...
				'view' => 'QuanLi/Service/Vip/quanli_reactions'
			);
			$this->load->view('QuanLi/layout', $data, FALSE);
		}
