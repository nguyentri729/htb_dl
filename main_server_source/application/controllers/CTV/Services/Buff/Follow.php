...
class Follow extends CI_Controller {
	...
	public function __construct(){
		...
		$this->load->model('ctv/service/buff/m_follow', 'm_follow');
		...
		if($this->input->get('tinh_tien') !=''){
			...
			exit(json_encode($this->m_follow->get_gia($sl, $loai, $this->info_member['type_acc'])));
		}
	}
	public function index()
	{
		if($this->input->post()){
			...
			$this->form_validation->set_rules('so_luong', 'Số lượng', 'required|integer|less_than_equal_to[999999]|greater_than_equal_to[300]');
			...
			if ($this->form_validation->run() == TRUE) {
				...
				$thanh_tien = $this->m_follow->get_gia($sl, $loai, $this->info_member['type_acc'])['tien_da_giam'];

				if($this->m_func->check_tien($thanh_tien, $this->session->userdata('ctv_id'), 'ctv')){
					//$this->load->model('m_token');
					$uid_page = $this->uid_page;
					$info = $this->m_token->get_name($uid_page, true);
					$name = $info['name'];
					$follow = $info['follow'];
					if($name == ''){
					...
					$server  = $this->m_server->get_url_server('server_buff', 'buff_follow');
					...
					if($this->db->insert('buff_follow', $data_insert)){
						...
						if($this->m_server->add($server['link'], $arrayName, $this->db->insert_id())){
							...
							$this->m_ctv->add_activity($this->session->userdata('ctv_id'), 'add', 'buff_follow', $thanh_tien, $this->db->insert_id());

						}else{
							$this->m_func->noti(false, 'Server con của chúng tôi đang gặp vấn đề ! Vui lòng thử lại sau ít phút !');
							$this->db->where('id', $this->db->insert_id());
							$this->db->delete('buff_follow');
						}
