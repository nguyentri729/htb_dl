...
class Reactions extends CI_Controller {
	...
	public function __construct(){
		...
		$this->load->model('ctv/service/buff/m_reactions', 'm_reactions');
		...
		if($this->input->get('tinh_tien') !=''){
			...
			exit(json_encode($this->m_reactions->get_gia($sl, $loai, $this->info_member['type_acc'])));
		}
	...
	public function index()
	{
		...
		if($this->input->post()){
			...
			$this->form_validation->set_rules('cam_xuc', 'Cảm xúc sử dụng', 'callback_check_camxuc');
			...
			if ($this->form_validation->run() == TRUE) {
				...
				$thanh_tien = $this->m_reactions->get_gia($sl, $loai, $this->info_member['type_acc'])['tien_da_giam'];

				if($this->m_func->check_tien($thanh_tien, $this->session->userdata('ctv_id'), 'ctv')){

					$this->load->model('m_server');
					$server  = $this->m_server->get_url_server('server_buff', 'buff_reactions');
					...
					if($this->db->insert('buff_reactions', $arrayName)){


						if($this->m_server->add($server['link'], $array_sever, $this->db->insert_id())){
							...
							$this->m_ctv->add_activity($this->session->userdata('ctv_id'), 'add', 'buff_reactions', $thanh_tien, $this->db->insert_id());
		...
		if($this->input->get('id_view') !=''){
			...
			if($this->m_captcha->check_captcha($captcha)['status']){
				...
				if($get->num_rows() > 0){
					...
					$status_arr = json_decode($this->m_func->curl_api($link_server.'/api/buff_reactions.php?type=view_status&id_main='.$id.'', 'GET'), true);
		...
		$data =  array(
			...
			'script' => 'script/CTV/Service/Buff/reactions',
			'view' => 'CTV/Service/Buff/reactions'
		);
