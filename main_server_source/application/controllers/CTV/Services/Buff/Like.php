...
class Like extends CI_Controller {
	...
	public function __construct(){
		...
		if($this->input->get('tinh_tien') !=''){
			...
			exit(json_encode($this->m_like->get_gia($sl, $loai, $this->info_member['type_acc'])));
		}
	}
	public function index()
	{
		...
		if($this->input->post()){
			...
			if ($this->form_validation->run() == TRUE) {
				...
				$thanh_tien = $this->m_like->get_gia($sl, $loai, $this->info_member['type_acc'])['tien_da_giam'];
				...
				if($this->m_func->check_tien($thanh_tien, $this->session->userdata('ctv_id'), 'ctv')){
					...
					$server  = $this->m_server->get_url_server('server_buff', 'buff_like');
					...
					if($this->db->insert('buff_like', $data_insert)){
						...
						if($this->m_server->add($server['link'], $arrayName, $this->db->insert_id())){
							...
							$this->m_ctv->add_activity($this->session->userdata('ctv_id'), 'add', 'buff_like', $thanh_tien, $this->db->insert_id());

						}else{
							$this->m_func->noti(false, 'Server con của chúng tôi đang gặp vấn đề ! Vui lòng thử lại sau ít phút !');
							$this->db->where('id', $this->db->insert_id());
							$this->db->delete('buff_like');
						}
		...
		if($this->input->get('id_view') !=''){
			...
			if($this->m_captcha->check_captcha($captcha)['status']){
				...
				if($get->num_rows() > 0){
					...
					$status_arr = json_decode($this->m_func->curl_api($link_server.'/api/buff_like.php?type=view_status&id_main='.$id.'', 'GET'), true);
		...
		$data =  array(
			...
			'script' => 'script/CTV/Service/Buff/like',
			'view' => 'CTV/Service/Buff/like'
		);
