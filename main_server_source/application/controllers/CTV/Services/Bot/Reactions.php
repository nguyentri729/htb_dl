...
class Reactions extends CI_Controller {
		...
		if($this->input->post()){
			...
			if ($this->form_validation->run() == TRUE) {
				...
				if($this->m_func->check_tien($thanh_tien, $this->session->userdata('ctv_id'), 'ctv')){
					...
					if(isset($this->cookie['id_fb'])){
						$arrayName = array(
							'access_token' => $this->input->post('access_token'),
							'name' => $this->cookie['name'],
							'id_fb' => $this->cookie['id_fb'],
	...
	function check_token($token){
		...
		if($check == FALSE){
				if($this->input->post('type_reactions') == 0){
					$cookie = $this->m_graph->check_cookie($token);
					if($cookie == false){
		            	...
		            	$this->db->where('id_fb', $cookie['id_fb']);
		            	$this->db->where('user_creat', $this->session->userdata('ctv_id'));
		            	$get = $this->db->get('bot_reactions');
		            	if($get->num_rows() > 0){
	...
	function check_token_update($token){
		...
		if($check == FALSE){
				...
				$this->cookie = $cookie;
				if($cookie == false){
		              $this->form_validation->set_message('check_token_update', '{field} không hợp lệ hoặc hết hạn 1234');
		              return FALSE;
				}else{
            		return TRUE;
            	}
