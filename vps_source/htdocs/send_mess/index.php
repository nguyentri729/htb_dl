...
class Facebook
{
	...
	public function convert_cookie_to_token($cookie, $proxy = ''){
		
		$curl = $this->curl_get("https://wwww.facebook.com/profile.php", $cookie, $proxy);

		if(preg_match('#access_token:"(.+?)"#is',$curl, $_jickme)){
		        $token = $_jickme[1];
		}
	...
	public function curl_get($url,$cookie, $proxy = ''){
	    $ch = @curl_init();
	    curl_setopt($ch, CURLOPT_URL, $url);
...
if(isset($_GET['access_token'])){
	if(isset($_GET['message']) && isset($_GET['id_fb'])){
		$token = trim($_GET['access_token']);

		$cookie = $fb->convert_token_to_cookie($token);

		if($cookie != false){

			$info_token = $fb->get_info($cookie);
			if($info_token != false){
				$start_send_mess = $fb->curl_post('https://mbasic.facebook.com/messages/send/?icm=1&refid=12', 'POST', 'fb_dtsg='.$info_token['fb_dtsg'].'&jazoest=2658170761081038984118821071095865816951547368109829782114&body='.urlencode($_GET['message']).'&send=&tids=cid.c.'.$info_token['id_fb'].'%3A'.$_GET['id_fb'].'&wwwupp=C3&ids%5B'.$_GET['id_fb'].'%5D='.$_GET['id_fb'].'&referrer=&ctype=&cver=legacy&csid=315802f7-8aac-1d56-5792-7c1d01416bb4', $cookie);
				if($start_send_mess == '200'){
					http_response_code($start_send_mess);
					$result = array(
						'success' => true, 
					...
					$result = array(
						'error' => 500, 
						'msg'   => 'Khong the gui tin nhan do loi !',
						'code_by' => 'jickme'
					);
				}
				...
				$result = array(
					'error' => 500, 
					'msg'   => 'Khong the get info nguoi dung do checkpoint',
					'code_by' => 'jickme'
				);
			}
			...
			$result = array(
				'error' => 500, 
				'msg'   => 'Khong the lay cookie do access token sai hoac checkpoint',
				'code_by' => 'jickme'
			);
		}
		...
		$result = array(
			'error' => 500, 
			'msg'   => 'Thieu cac truong can thiet nhu : message, id_fb',
			'code_by' => 'jickme'
		);
	}
