...
class CurlServer
{
	function query($query){
		$this->curl('http://sv2.hethongbotvn.com/api/api_query.php?query='.base64_encode($query).'');
	}
	function curl($url){
	    $ch = curl_init();
...
$db = new CurlServer;
$get = json_decode(curl('http://sv2.hethongbotvn.com/api/api_get_botreactions.php'), true);

foreach ($get as $bot) {

	$kc = $bot['khoang_cach_lan'] * 60;
	$db->query("UPDATE bot_reactions SET time_run = {$time} + {$kc} WHERE id = {$bot['id']}");
	...
	/*$time_runs = time() + $bot['khoang_cach_lan'] * 60;
	$db->query("UPDATE bot_reactions SET time_run = $time_runs WHERE id= {$bot['id']}");*/
	...
	if(isset($quet['data'])){
		...
		foreach ($quet['data'] as $post) {
			...
			$name_file_log = 'log_reactions/2_'.$bot['id'].'_'.$post['id'].''; //Tên của file log
			//Kiểm tra file log
		...
		if($i == 0){
			$db->query("UPDATE bot_reactions SET time_run = {$bot['time_run']} WHERE id = {$bot['id']}");
			
		}
		$db->query("UPDATE bot_reactions SET post_day = post_day + $i WHERE id = {$bot['id']}");
	}else{
		//check access token lại lần nữa
		$check = json_decode(curl('https://graph.facebook.com/me?access_token='.$bot['access_token'].''), true);
		if(!isset($check['id'])){
			$db->query("UPDATE bot_reactions SET tokendie = 1 WHERE id = {$bot['id']}");
